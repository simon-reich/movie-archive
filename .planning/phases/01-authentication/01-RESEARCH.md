...
ResponseCookie cookie = ResponseCookie.from("refresh_token", rawRefreshToken)
    ...
    .path("/api/auth/refresh")  // browser-visible path; Caddy strips /api before forwarding to Spring
    .build();
...
public LoginResponse login(LoginRequest req, HttpServletResponse response) {
    ...
    ResponseCookie cookie = ResponseCookie.from("refresh_token", rawRefresh)
        ...
        .maxAge(Duration.ofDays(7)).path("/api/auth/refresh").build();
    response.addHeader(HttpHeaders.SET_COOKIE, cookie.toValueString());
...
1. **Cookie path scope with Caddy proxy**
   ...
   - What was unclear: Does the browser send the cookie to `/api/auth/refresh` (Caddy path) or does the cookie path need to be `/api/auth/refresh`?
   - **RESOLVED:** Caddy does NOT strip the `/api` prefix by default — `reverse_proxy` forwards the full path. The Caddyfile `handle /api/*` block must have `uri strip_prefix /api` added so Spring Boot controllers receive `/auth/**` (consistent with existing SecurityConfig `permitAll` rules). With that fix in place, the browser calls `POST /api/auth/refresh` (the full path). The `Set-Cookie` header is generated by Spring Boot and the `path` attribute is what the **browser** sees — not the upstream path. Therefore the cookie path must be `path(/api/auth/refresh)` (not `/auth/refresh`), so the browser sends the cookie when it calls the Caddy-fronted URL.
