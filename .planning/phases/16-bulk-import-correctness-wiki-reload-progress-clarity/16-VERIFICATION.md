...
| 12 | Movies whose Wikipedia page has already been found are not re-selected for wiki-reload retry forever (G-16-3, closed by 16-04) | ✓ VERIFIED | `MovieRepository.findEligibleForWikiReload` now keys eligibility on `wiki_url IS NULL` instead of `wiki_plot`/`wiki_critics IS NULL`. `WikiReloadServiceIntegrationTest` regression test passes (20/20 targeted tests). Live UAT (16-UAT.md, Test 3, 2026-08-29): user could not reproduce a genuine NOT_FOUND case live to re-exercise the icon path directly — expected, since the fix removed the endless-retry condition that made the case easy to hit. Accepted as pass on the strength of truth #9's already-passing NOT_FOUND-icon regression test; revisit live if a genuine NOT_FOUND case resurfaces. |
