...
class ImportLineParserTest {
    ...
    // -------------------------------------------------------------------------
    // 15-03: parseCsv() — comma-delimited CSV sibling parser (D-12/D-13/D-15)
    // -------------------------------------------------------------------------

    @Test
    void shouldReturnNull_forBlankCsvLine() {
        assertThat(parser.parseCsv("   ")).isNull();
    }

    @Test
    void shouldParseValidCsvLine_withEmptyOriginalTitle() {
        ImportLineParser.ParsedLine parsed = parser.parseCsv("Inception,,2010");
    ...
    @Test
    void shouldParseValidCsvLine_withOriginalTitle() {
        ImportLineParser.ParsedLine parsed = parser.parseCsv("Robin Hood,Robin des Bois,2010");
    ...
    @Test
    void shouldParseQuotedCommaContainingTitle_asOneField() {
        ImportLineParser.ParsedLine parsed = parser.parseCsv("\"Title, Part 2\",,2010");

        assertThat(parsed.valid()).isTrue();
        assertThat(parsed.title()).isEqualTo("Title, Part 2");
        assertThat(parsed.originalTitle()).isNull();
        assertThat(parsed.year()).isEqualTo(2010);
    }

    @Test
    void shouldBeInvalid_whenCsvFieldCountIsWrong() {
        ImportLineParser.ParsedLine parsed = parser.parseCsv("Title,2010");
    ...
    @Test
    void shouldBeInvalid_whenCsvYearIsNonNumeric() {
        ImportLineParser.ParsedLine parsed = parser.parseCsv("Title,,abcd");
